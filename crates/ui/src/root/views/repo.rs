        ...
        div { class: "mx-auto max-w-6xl px-4 py-6 space-y-6",
            div { class: "flex items-center justify-between gap-4",
                div { class: "space-y-1 min-w-0",
                    ...
                    p { class: "text-sm text-[color:var(--secondary-color-5)]", "repo detail" }
                }

                div { class: "flex items-center gap-2",
                    button {
                        class: "text-sm text-[color:var(--secondary-color-5)] hover:underline",
                        onclick: move |_| {
                            navigator.go_back();
                        },
                        "返回"
                    ...
                    button {
                        class: "inline-flex items-center justify-center rounded-md border border-[color:var(--primary-color-6)] bg-[color:var(--primary-color-1)] px-3 py-2 text-sm hover:bg-[color:var(--primary-color-4)]",
                        onclick: move |_| {
                            repo_fut.restart();
            ...
            section { class: "rounded-xl border border-[color:var(--primary-color-6)] bg-[color:var(--primary-color-3)] p-4 space-y-3",
                h2 { class: "text-lg font-semibold", "Repo" }
                match repo_fut() {
                    Some(Ok(Some(repo))) => rsx! {
                        div { class: "grid grid-cols-1 gap-2 text-sm sm:grid-cols-2",
                            div { class: "flex items-center justify-between",
                                span { class: "text-[color:var(--secondary-color-5)]", "id" }
                                span { class: "font-medium", "{repo.id}" }
                            }
                            div { class: "flex items-center justify-between",
                                span { class: "text-[color:var(--secondary-color-5)]", "github_repo_id" }
                                span { class: "font-medium", "{repo.github_repo_id:?}" }
                            }
                            div { class: "flex items-center justify-between",
                                span { class: "text-[color:var(--secondary-color-5)]", "stars" }
                                span { class: "font-medium", "{repo.stars}" }
                            }
                            div { class: "flex items-center justify-between",
                                span { class: "text-[color:var(--secondary-color-5)]", "forks" }
                                span { class: "font-medium", "{repo.forks}" }
                            }
                            div { class: "flex items-center justify-between",
                                span { class: "text-[color:var(--secondary-color-5)]", "open_issues" }
                                span { class: "font-medium", "{repo.open_issues}" }
                            }
                            div { class: "flex items-center justify-between",
                                span { class: "text-[color:var(--secondary-color-5)]", "watchers" }
                                span { class: "font-medium", "{repo.watchers}" }
                            }
                        }
                        if let Some(full_name) = repo.full_name {
                            div { class: "text-sm text-[color:var(--secondary-color-5)]", "full_name: {full_name}" }
                        }
                        if let Some(last) = repo.last_fetched_at {
                            div { class: "text-sm text-[color:var(--secondary-color-5)]", "last_fetched_at: {last}" }
                        }
                    },
                    Some(Ok(None)) => rsx! {
                        div { class: "text-sm text-[color:var(--secondary-color-5)]", "未找到 repo" }
                    },
                    Some(Err(e)) => Err(e)?,
                    None => rsx! {
                        div { class: "text-sm text-[color:var(--secondary-color-5)]", "Loading..." }
                    },
                }
            }
            ...
            div { class: "grid grid-cols-1 gap-6 lg:grid-cols-2",
                section { class: "rounded-xl border border-[color:var(--primary-color-6)] bg-[color:var(--primary-color-3)] p-4 space-y-3",
                    ...
                    match snapshots_fut() {
                        Some(Ok(page)) => rsx! {
                            div { class: "text-sm text-[color:var(--secondary-color-5)]", "count: {page.meta.total}" }
                            div { class: "max-h-[520px] overflow-auto space-y-2",
                                for s in page.items {
                                    div {
                                        key: "{s.snapshot_date}",
                                        class: "rounded-md border border-[color:var(--primary-color-6)] bg-[color:var(--primary-color-2)] px-3 py-2 text-sm",
                                        div { class: "flex items-center justify-between",
                                            span { class: "font-medium", "{s.snapshot_date}" }
                                            span { class: "text-[color:var(--secondary-color-5)]", "stars: {s.stars}" }
                                        }
                                ...
                                for d in page.items {
                                    div {
                                        ...
                                        div { class: "flex items-center justify-between",
                                            ...
                                            span { class: "text-[color:var(--secondary-color-5)]", "prev: {d.prev_snapshot_date:?}" }
                                        }
                                        div { class: "text-xs text-[color:var(--secondary-color-5)]",
