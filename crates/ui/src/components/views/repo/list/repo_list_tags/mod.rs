...
use super::{
    ...
    RepoListTagsCachedState, TagAdviceItem,
};
    ...
    match facets() {
        Some(Ok(items)) => {
            ...
            let advice_tags = items
                ...
                .collect::<Vec<_>>();
            let cached = RepoListTagsCachedState {
                advice_tags: advice_tags.clone(),
            };
            if (ctx.last_success_tags)().as_ref() != Some(&cached) {
                ctx.last_success_tags.set(Some(cached));
            }

            rsx! {
                RepoListTagsContent { advice_tags }
            }
        }
        Some(Err(_)) => {
            ctx.last_success_tags.set(None);
            rsx! {}
        }
        None => rsx! { skeleton::RepoListTagsCachedFallback {} },
    }
}

#[component]
pub(super) fn RepoListTagsContent(advice_tags: Vec<TagAdviceItem>) -> Element {
