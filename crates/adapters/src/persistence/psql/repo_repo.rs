...
impl RepoRepo for PostgresRepoRepo {
    ...
    async fn list_with_filter(&self, tags: &[String], metric: Option<RepoRankMetric>, range: Option<RepoRankTimeRange>, page: Pagination) -> AppResult<Page<Repo>> {
        ...
        let in_clause = vec!["?"; tags.len()].join(", ");
        let count_sql = format!(
            "SELECT COUNT(*) FROM (SELECT r.id FROM repos r \
             JOIN repo_tag_map m ON m.repo_id = r.id \
             JOIN tags t ON t.id = m.tag_id \
             WHERE t.value IN ({}) \
             GROUP BY r.id \
             HAVING COUNT(DISTINCT t.value) = {}) AS filtered",
            in_clause, tags.len()
        );
        
        let mut final_count_qb: QueryBuilder<Postgres> = QueryBuilder::new(count_sql);
        for tag in tags { final_count_qb.push_bind(tag); }

        let total: i64 = final_count_qb
            .build_query_scalar()
            .fetch_one(&self.pool)
            .await
            .map_err(db_err)?;
        ...
        // Main Query
        let anchor_date: Option<chrono::NaiveDate> = if range.is_some() {
            sqlx::query_scalar("SELECT MAX(snapshot_date) FROM snapshot_deltas")
                .fetch_one(&self.pool)
                .await
                .map_err(db_err)?
        ...
        if let (Some(m), Some(r), Some(anchor_date)) = (metric, range, anchor_date) {
            ...
            qb.push(r#"
                SELECT
                  ...
                  COALESCE(SUM(d.forks_delta), 0)::BIGINT AS forks,
                  ABS(COALESCE(SUM(d.open_issues_delta), 0))::BIGINT AS open_issues,
                  r.watchers, r.created_at::TEXT as created_at, r.last_fetched_at, r.etag
                FROM repos r
                JOIN repo_tag_map m ON m.repo_id = r.id
                JOIN tags t ON t.id = m.tag_id
                LEFT JOIN snapshot_deltas d
            ...
            qb.push(r#"
                SELECT
                  ...
                  r.homepage_url, r.avatar_url,
                  r.stars, r.forks, r.open_issues, r.watchers,
                  r.created_at::TEXT as created_at, r.last_fetched_at, r.etag
                FROM repos r
                JOIN repo_tag_map m ON m.repo_id = r.id
                JOIN tags t ON t.id = m.tag_id
                WHERE t.value IN ("#);
