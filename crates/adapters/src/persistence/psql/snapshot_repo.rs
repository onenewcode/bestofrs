...
impl SnapshotRepo for PostgresSnapshotRepo {
    ...
    async fn list_by_repo(&self, repo_id: &RepoId, page: Pagination) -> AppResult<Page<Snapshot>> {
        ...
        let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM snapshots WHERE repo_id = $1")
            .bind(repo_id.as_str())
            .fetch_one(&self.pool)
            .await
            .map_err(db_err)?;
...
#[async_trait::async_trait]
impl SnapshotDeltaRepo for PostgresSnapshotRepo {
    ...
    async fn upsert_for_date(
        ...
        let mut builder: QueryBuilder<Postgres> = QueryBuilder::new(
            ...
            FROM snapshots s
            LEFT JOIN LATERAL (
                SELECT snapshot_date, stars, forks, open_issues, watchers
                FROM snapshots s2
